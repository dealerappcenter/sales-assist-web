      ...
      <Section className='lg:px-0'>
        <div className='flex justify-center flex-col gap-6'>
          <div className='flex flex-col pb-12'>
            ...
            <div className='flex flex-row gap-2 md:gap-4 lg:leading-[5.375rem]'>
              <h1 className='md:text-6xl text-orange-normal font-normal'>{heroData.hero.lower_message}</h1>
              <div id='words' ref={el => divRef.current = el} className='overflow-hidden relative gap-6 flex flex-col h-[52px] md:h-[65px] lg:h-[80px] slices'>
                {words.map((m, i) => <h1 key={m + i} className='text-orange-normal md:text-6xl slice'>{m}</h1>)}
              </div>
            </div>
          </div>
