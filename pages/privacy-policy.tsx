  ...
  <>
    ...
    <section id='policy'>
      <Section className="container mx-auto py-12 flex flex-col gap-6 text-gray-primary">
        ...
        {/* SMS Communication & Customer Support Section */}
        <div className="flex flex-col gap-6">
          <h1 className="font-bold text-xl">{PolicyData?.data.sms_communication.name}</h1>
          {PolicyData?.data.sms_communication.description.map(paragraph => <a className="" key={paragraph}>{paragraph}</a>)}
        </div>
        {/*  */}
        <div className="flex flex-col gap-6">
